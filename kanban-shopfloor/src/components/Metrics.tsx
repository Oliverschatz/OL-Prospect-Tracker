import { ANON_PREFIX, Board } from '../types';
...
// Resolve which org an assignee id belongs to (handles the `anon:<orgId>` form).
function orgIdOfAssignee(board: Board, a: string): string | null {
  if (a.startsWith(ANON_PREFIX)) return a.slice(ANON_PREFIX.length);
  const node = nodeById(board, a);
  if (!node) return null;
  return node.kind === 'org' ? node.id : orgOf(board, node)?.id ?? null;
}

export default function Metrics({ board, onClose }: { board: Board; onClose: () => void }) {
  const cards = liveCards(board);
  ...
  // Schedule health.
  const warned = cards.filter(c => cardWarnings(c).length > 0).length;

  // Workload per organization (a card counts once per org it touches).
  ...
  for (const c of cards) {
    ...
    for (const a of c.assignees) {
      const oid = orgIdOfAssignee(board, a);
      if (oid) touched.add(oid);
    }
      ...
      <div className="modal modal-wide" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="metric-grid">
            <div className="stat"><span className="stat-num">{cards.length}</span><span className="stat-lbl">cards</span></div>
          ...
          {method === 'three_point' && (
            <p className="metric-line">
              Expected (PERT) — time <strong>{round(tp.time)}</strong> · workload <strong>{round(tp.workload)}</strong> · cost <strong>{round(tp.cost)}</strong>
            </p>
          )}
          ...
          <div className="bars">
            {orgs.map(o => (
              <div className="bar-row" key={o.id}>
                ...
                <span className="bar-track"><span className="bar-fill" style={{ width: `${((orgLoad.get(o.id) ?? 0) / Math.max(1, cards.length)) * 100}%`, background: o.color }} /></span>
            ...
            {unassigned > 0 && (
              <div className="bar-row">
                ...
                <span className="bar-track"><span className="bar-fill" style={{ width: `${(unassigned / Math.max(1, cards.length)) * 100}%`, background: '#aab2c0' }} /></span>
          ...
          {cards.length > 0 && (
            <p className="metric-foot muted">
              ...
              {(() => {
                ...
                for (const c of cards) for (const a of c.assignees) counts.set(a, (counts.get(a) ?? 0) + 1);
